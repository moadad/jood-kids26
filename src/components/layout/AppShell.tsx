...
import { ShoppingBag, Home, LayoutGrid, Shield, Menu, X, CalendarDays, Layers, Info, FileText, Truck } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isUnlocked = typeof document !== "undefined" && document.cookie.includes("admin_unlocked=1");

  function onBrandClick() {
    // 7 clicks on the logo/title unlock admin link for this browser
    setAdminClicks((c) => {
      const next = c + 1;
      if (next >= 7) {
        try {
          document.cookie = "admin_unlocked=1; path=/; max-age=31536000";
        } catch {}
      }
      return next;
    });
      ...
      <div className="mx-auto max-w-md min-h-dvh bg-white shadow-soft">
        <div className="p-4 border-b border-zinc-100 sticky top-0 bg-white/90 backdrop-blur z-20">
          <div className="flex items-center justify-between">
            ...
            <div className="font-extrabold text-brand-800 tracking-tight cursor-pointer select-none" onClick={onBrandClick} title="انقر 7 مرات لفتح الإدارة">JoodKids</div>
              ...
              <div className="absolute top-0 right-0 left-0 mx-auto max-w-md">
                <div className="m-3 rounded-3xl bg-white shadow-soft border border-zinc-100 overflow-hidden">
                  ...
                  <div className="p-2 grid gap-2">
                    ...
                    <Link onClick={() => setMenuOpen(false)} className="flex items-center justify-between rounded-2xl border border-zinc-200 p-3 hover:bg-zinc-50" href="/track">
                      <span className="flex items-center gap-2"><Truck size={18} /> تتبع الطلب</span>
                      <span className="text-xs text-zinc-500">برقم الطلب</span>
                    </Link>
                  </div>
                </div>
              </div>
