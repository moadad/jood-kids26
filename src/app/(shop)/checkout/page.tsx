...
function buildWhatsAppMessage(orderId: string, orderNumber: string, name: string, phone: string, gov: string, city: string, address: string, shipName: string, payment: string, items: any[], total: number) {
  const lines = [
    `طلب جديد - JoodKids`,
...
function generateOrderNumber() {
  // رقم قصير سهل القراءة (10 أرقام): آخر 8 أرقام من الوقت + رقمين عشوائيين
...
if (!paymentMethod) {
  ...
  return;
    ...
    try {
      ...
      // مستند تتبع عام (للتتبع برقم الطلب بدون كشف جميع الطلبات)
      await setDoc(doc(db, "orderTracking", orderNumber), {
        orderId: docRef.id,
        orderNumber,
        status: "new",
        customerName: name.trim(),
      ...
      if (whatsappNumber) {
        ...
        const msg = buildWhatsAppMessage(docRef.id, orderNumber, name.trim(), phone.trim(), governorate.trim(), city.trim(), address.trim(), shipName, paymentMethod, items, total);
      ...
      <Card>
        <CardContent className="p-5 space-y-3">
          ...
          <select className="h-11 rounded-2xl border border-zinc-200 px-3 text-sm bg-white" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
            ...
            <option value="wallets">محافظ إلكترونية (فودافون كاش / اتصالات كاش / أورنج كاش)</option>
          </select>
